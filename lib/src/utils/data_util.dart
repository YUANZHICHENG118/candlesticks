...
class DataUtil {
  static calculate(List<Candle> dataList,
    ...
    calcKDJ(dataList);
    calcMACD(dataList);
    calculateRSI(dataList, 6, (candle, rsi) => candle.rsi1 = rsi);
    calculateRSI(dataList, 12, (candle, rsi) => candle.rsi2 = rsi);
    calculateRSI(dataList, 24, (candle, rsi) => candle.rsi3 = rsi);
    calcWR(dataList);
    calcCCI(dataList);
    return dataList;
  }

  static calcMA(List<Candle> dataList, List<int> maDayList) {
    List<double> ma = List<double>.filled(maDayList.length, 0);

    if (dataList.isNotEmpty) {
      for (int i = 0; i < dataList.length; i++) {
        Candle entity = dataList[i];
  ...
  static void calcBOLL(List<Candle> dataList, int n, int k) {
  ...
  static void _calcBOLLMA(int day, List<Candle> dataList) {
    double ma = 0;
    for (int i = 0; i < dataList.length; i++) {
      Candle entity = dataList[i];
      ma += entity.close;
  ...
  static void calcMACD(List<Candle> dataList) {
    double ema12 = 0;
    double ema26 = 0;
    double dif = 0;
    double dea = 0;
  ...
  static void calcVolumeMA(List<Candle> dataList) {
    double volumeMa5 = 0;
    double volumeMa10 = 0;

    for (int i = 0; i < dataList.length; i++) {
      Candle entry = dataList[i];
  ...
  static void calculateRSI(List<Candle> dataList, int period, Function(Candle, double?) setRSI) {
    double? rsi;
    double rsiABSEma = 0;
    double rsiMaxEma = 0;
    for (int i = 0; i < dataList.length; i++) {
  ...
  static void calcKDJ(List<Candle> dataList) {
    var preK = 50.0;
    var preD = 50.0;
  ...
  static void calcWR(List<Candle> dataList) {
    double r;
    for (int i = 0; i < dataList.length; i++) {
      Candle entity = dataList[i];
      int startIndex = i - 14;
  ...
  static void calcCCI(List<Candle> dataList) {
